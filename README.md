...
Este repositorio corresponde la prueba técnica para acceder a las prácticas de empresa. 
...
> ⚠️ Es necesario tener permisos root **solo para los siguientes dos comandos**.

Se debe de **asegurar** que se está lanzando *Docker*. Se puede comprobar con:
```
sudo systemctl status docker
```
Si no se encuentra lanzado, se puede lanzar de la siguiente forma:
```
...
/Tecnica
  ...
  │   ├── kedro_data_pipeline.py      # Ejecuta el pipeline de Kedro con 'kedro run'
...
*Kedro* cuenta con una herramienta llamada ***Kedro Viz*** que permite ver graficamente los pipelines y flujos existentes. Se tenía planificado realizar una DAG de *Airflow*, pero se ha dificultado su integración, aun así, se puede ejecutar con este comando nada mas levantar los contenedores:

```
docker exec -it kedro_container bash -c "cd kedro_project && kedro viz --port 4141 --host 0.0.0.0"
```

Y se puede acceder a él desde el navegador con:

```
http://127.0.0.1:4141/
```

El problema principal con *Kedro* fue cómo manejar los *volúmenes* para que se sincronizaran correctamente los cambios, además de problemas de permisos relacionados con la generación de su imagen. Finalmente, se pudo arreglar y funciona correctamente, sincronizandose los volúmenes correctamente.
...
Nuevamente, el tema de los permisos ha sido un problema, y es a raíz de este fichero *docker-compose.yml*. También, sé que hay partes que quizá podrían estar un poco mejor hechas, pero que tuve que dejar así dado que me estaba retrasando demasiado y no había avanzado en otras.
...
* Pruebas unitarias para cada parte, para demostrar el correcto funcionamiento de forma rápida e identificar fallos rápidamente.
* Diagrama profesional con el funcionamiento del proyecto. Diviendo sus partes y mostrando cómo se comunica todo.
